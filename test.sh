...
id=$(docker run -dP --runtime=nvidia -e LD_LIBRARY_PATH=/usr/lib/nvidia -v /usr/lib/nvidia-384:/usr/lib/nvidia:ro -v /tmp/.X11-unix:/tmp/.X11-unix:ro --name test -e X11VNC_PASSWORD=$password -v `pwd`/test:/input:ro soichih/vncserver-mrview)
